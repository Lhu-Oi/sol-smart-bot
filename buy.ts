...
import { COIN_ADDRESS } from './constants';
import { fetchTransaction } from './utils/transaction-v2';
...
async function wsCoin(): Promise<void> {
  let count = 0;
  ...
  ws.on('message', async (data: string) => {
    ...
    if (response.result) {
      ...
      await processCoinSwapActivity(signature)
      console.log(`============================================================================================${count}-New update received on ${formattedDate} ${formattedTime} ============================================================================================`);
...
async function processCoinSwapActivity(signature: string) {
  ...
  if (item_activity && Object.keys(item_activity).length !== 0) {
    ...
    if (item_activity.data.token_1 === SOL_ADDRESS || item_activity.data.token_1 === USDC_ADDRESS) {
      const pool_info = await solanaConnection.getAccountInfoAndContext(new PublicKey(ammId))
      const poolState = LIQUIDITY_STATE_LAYOUT_V4.decode(pool_info.value!.data);

      if (!seenSignatures.has(ammId)) {
        seenSignatures.add(ammId);
        await buy(new PublicKey(ammId), poolState);
      }
    }
  }
}
